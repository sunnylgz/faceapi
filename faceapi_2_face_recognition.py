...
def crop_face(img):
  ...
  #bounding_boxes = face_recognition.face_locations(img, number_of_times_to_upsample=0, model="cnn")
  bounding_boxes = face_recognition.face_locations(img, number_of_times_to_upsample=0)
...
def face_compare(image1,
  ...
  bounding_boxes = []
  try:
    prewhitened1,bb1 = crop_face(img1)
  ...
  try:
    prewhitened2,bb2 = crop_face(img2)
